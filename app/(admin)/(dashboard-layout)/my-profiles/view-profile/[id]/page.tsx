...
const Profile = ({ params, searchParams }: PageProps) => {
  ...
  const fetchProfileData = useCallback(async () => {
    ...
    try {
      if (profileType === 'Business') {
        response = await getBusinessProfileDetailsByUserId(params.id);
      } else if (profileType === 'Individual') {
        response = await getIndividualProfileDetailsByUserId(params.id);
      } else {
        throw new Error('Invalid profile type');
      }

      if (response.success) {
        dispatch(addProfileData(response.data[0]));
      } else {
  ...
  }, [currentUser, dispatch, params.id]);
    ...
    <div className="px-4 py-6 md:p-6 overflow-x-hidden">
      <div className="flex items-center justify-between gap-4 mb-8">
        ...
        <div className="inline-flex items-center gap-1.5">
          <Share className="text-omblue-600 p-2 rounded-full w-8 h-8 border border-omblue-100" />
          <EditIcon className="text-omblue-600 p-2 rounded-full w-8 h-8 border border-omblue-100" />
          <Trash className="text-danger-600 p-2 rounded-full w-8 h-8 border border-danger-100" />
        </div>
      </div>
      <div className="grid grid-cols-12 gap-10 mb-20">
        <div className="col-span-12 md:col-span-6">
          <div className="flex items-start gap-6">
            <div className="bg-profile-frame p-1 w-20 h-20 md:w-36 md:h-36 rounded-full flex items-center justify-center relative">
              <Image
                src={`${process.env.NEXT_PUBLIC_MEDIA_URL}/user-profile/${profileData.BasicInfo.ProfilePicture}`}
            ...
            <div className="space-y-3 mt-1">
              <div className="flex items-center gap-2.5">
                ...
                <Tooltip content={profileData.BasicInfo.CountryOfOrigin}>
                  {/* <Image
                  ...
                  /> */}
                  <>{profileData.BasicInfo.CountryOfOrigin}</>
                </Tooltip>
              </div>
              <p className="text-h7 2xl:text-scaled-h7 font-medium text-blue-300">
                {profileData.BasicInfo.Tagline ||
                  profileData.BasicInfo.WorkTitle}
              </p>
              <div className="flex items-center gap-2">
                {icons[currentUser.UserProfileType]}
        ...
        <div className="col-span-12 md:col-span-6 space-y-4">
          <h6 className="text-h6 2xl:text-scaled-h6 font-semibold text-blue-500">
            About{' '}
            {profileData.BasicInfo.CompanyName || profileData.BasicInfo.Name}
          </h6>
          <div className="text-black-300 text-h8 2xl:text-scaled-h8 font-medium space-y-4">
            <p className="">
              {profileData.AboutCompany || profileData.BasicInfo.AboutYou}
            </p>
